...
int rtlsdr_read_async_from_file(rtlsdr_dev_t *dev, rtlsdr_read_async_cb_t cb, void *ctx,
			  uint32_t buf_num, uint32_t buf_len)
{
	unsigned int i;
	int r = 0;
...
int rtlsdr_read_async_timeout(rtlsdr_dev_t *dev, rtlsdr_read_async_cb_t cb, void *ctx,
			  uint32_t buf_num, uint32_t buf_len)
{
	...
	while (RTLSDR_INACTIVE != dev->async_status) {
		...
		if (t_end.tv_sec - t_start.tv_sec > 5){
			r = 5;
			break;
		}

#ifdef HAVE_LIBUSB_HANDLE_EVENTS_TIMEOUT_COMPLETED
		r = libusb_handle_events_timeout_completed(NULL, &tv, &dev->async_cancel);
#else
		...
		if (RTLSDR_CANCELING == dev->async_status) {
			...
			if (dev->dev_lost || RTLSDR_INACTIVE == next_status) {
				 ...
				 * just cancelled all transfers */
#ifdef HAVE_LIBUSB_HANDLE_EVENTS_TIMEOUT_COMPLETED
				libusb_handle_events_timeout_completed(NULL, &zerotv, NULL);
#else
				libusb_handle_events_timeout(NULL, &zerotv);
#endif
				break;
			}
		}
	...
	if (r ==5){
		printf("rtl_test passed, device is working\n");
	while (RTLSDR_INACTIVE != dev->async_status) {
